#!/usr/bin/python3
"""
Phoenix v6 — 最大股票池 + ClickHouse验证参数
目标: 复现MA250+5%月止损+ret_1m=高年化低回撤
参数对齐ClickHouse bt_timing_v3.py最优结果
"""
...
TOP_N = 10
MA_SLOW = 250
STOP_LOSS = 0.05        # ClickHouse验证: 5%月止损
...
def compute_factors(stock_data):
    ...
    return factor_df.dropna(subset=['ret_1m'])

def run_backtest(index_df, factor_df, stock_data):
    print("\n" + "="*60)
    print("🔥 Phoenix v6 — 200只股票池 + 纯MA250 + 5%止损")
    print("="*60)
...
def main():
    print("="*60)
    print("🔥 Phoenix v6 — 200只池 + ClickHouse参数")
    print("="*60)
    ...
    with open('/opt/quant/phoenix_v6_result.json', 'w') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    records.to_csv('/opt/quant/phoenix_v6_daily.csv', index=False)
    print(f"\n结果: /opt/quant/phoenix_v6_result.json")
