#!/usr/bin/python3
"""
Phoenix v7 — v6基础 + 周度止损检查
改进: 止损检查从21天→5天, 其他不变
预期: 回撤大幅下降, 收益略降(更多假触发)
"""
...
END_DATE = "20260618"
REBALANCE_FREQ = 21
STOP_CHECK_FREQ = 5    # 每5天检查一次止损(v6是21天)
TOP_N = 10
MA_SLOW = 250
STOP_LOSS = 0.05
...
def run_backtest(index_df, factor_df, stock_data):
    print("\n" + "="*60)
    print("🔥 Phoenix v7 — 周度止损检查(5天)")
    print("="*60)
    
    idx = index_df.copy()
    idx['ma250'] = idx['close'].rolling(MA_SLOW).mean()
    idx['timing'] = (idx['close'] > idx['ma250']).astype(int)
    
    dates = idx['date'].tolist()
    start_i = MA_SLOW
    rebalance_dates = set(dates[i] for i in range(start_i, len(dates), REBALANCE_FREQ))
    # 止损检查日: 每5天
    stop_check_dates = set(dates[i] for i in range(start_i, len(dates), STOP_CHECK_FREQ))
    ...
    for i, date in enumerate(dates):
        ...
        timing = idx.iloc[i]['timing']
        
        # 周度止损检查(每5天)
        if date in stop_check_dates:
            period_ret = nav / last_check_nav - 1
            if period_ret < -STOP_LOSS and portfolio:
                stop_active = True
                n_stop += 1
                print(f"  ⚠️ 止损: {date.date()}, 5日收益={period_ret*100:.1f}%")
    ...
    print(f"  止损触发: {n_stop}次")
    return pd.DataFrame(daily_records)

def analyze(records_df, index_df):
    print("\n" + "="*60)
    print("📊 Phoenix v7 Results")
    print("="*60)
    ...
    print(f"\n📊 仓位: 平均{exposure_avg*100:.1f}%  择时在场{timing_pct:.1f}%")
...
def main():
    ...
    with open('/opt/quant/phoenix_v7_result.json', 'w') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    records.to_csv('/opt/quant/phoenix_v7_daily.csv', index=False)
    print(f"\n结果: /opt/quant/phoenix_v7_result.json")
